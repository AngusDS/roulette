...
const prizeConfig = [
  ...
  { name: '謝謝參與沒料', percent: 10, color: '#3313ebff' },
  { name: '請你吃飯', percent: 15, color: '#ff5656ff' },
...
function showResult() {
  ...
  let idx = Math.floor(eff / arc);
  resultDiv.textContent = `恭喜獲得：${prizes[idx]}`;
}
...
function spinWheel() {
  ...
  function animate(ts) {
    ...
    const t = Math.min(progress / 3000, 1); // 3 秒
